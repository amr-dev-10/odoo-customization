...
class AddEmi(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        max_duration = float(config_param.sudo().get_param("installment.max_duration", default=0.0))

        if max_duration == 0:
            raise UserError("Max Duration is zero.")

        down_payment_percentage = int(config_param.sudo().get_param("installment.down_payment", default=0))
        annual_rate = int(config_param.sudo().get_param("installment.annual_rate", default=0))
        administrative_expenses = int(config_param.sudo().get_param("installment.administrative_expenses", default=0.0))
