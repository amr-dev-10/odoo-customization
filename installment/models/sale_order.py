...
class SalesOrder(models.Model):
    ...
    def document_upload(self):
        ...
        existing_workspace = self.env["documents.folder"].sudo().search(
            [("name", "=", workspace_name)], limit=1
        )

        # Create workspace folder if it doesn't exist
        if not existing_workspace:
            existing_workspace = self.env["documents.folder"].sudo().create({
                "name": workspace_name,
        ...
        existing_sub_folder = self.env["documents.folder"].sudo().search(
            [("name", "=", sub_folder), ("parent_folder_id", "=", existing_workspace.id)], limit=1
        )

        # Create subfolder if it doesn't exist
        if not existing_sub_folder:
            existing_sub_folder = self.env["documents.folder"].sudo().create({
                "name": sub_folder,
            ...
            exiting_doc_list = self.env["documents.document"].sudo().search(
    ...
    def _get_document_list(self):
        ...
        if config_param.sudo().get_param("installment.nid", default=False):
            document_list.append("National ID (NID)")
        if config_param.sudo().get_param("installment.salary_components", default=False):
            document_list.append("Salary Components")
        if config_param.sudo().get_param("installment.bank_statement", default=False):
            document_list.append("Bank Statement")
        if config_param.sudo().get_param("installment.bank_rate_letter", default=False):
            document_list.append("Bank Rate Letter")
        if config_param.sudo().get_param("installment.rental_contract", default=False):
            document_list.append("Rental Contract")
        if config_param.sudo().get_param("installment.ownership_contract", default=False):
