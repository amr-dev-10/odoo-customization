...
class AddWarranty(models.TransientModel):
    _name = "warranty.add"
    _description = "Warranty Add Wizard"
    ...
    def add_warranty_wizard_action(self):
        ...
        for record in self.product_ids:
            ...
            if record.year_id:
                ...
                sale_order_line = sale_order.order_line.filtered(lambda line: line.id == record.product_id.id)

                price = sale_order_line.price_subtotal * (record.year_id.percentage / 100)

                sale_order.order_line.create({
                    "name": f"{record.year_id.product_id.name} End Date: {record.end_date}",
                    "order_id": sale_order.id,
                    "price_unit": price,
                    "product_id": record.year_id.product_id.id,
